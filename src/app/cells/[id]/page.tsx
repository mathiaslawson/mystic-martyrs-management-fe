...
async function getZoneData(id: string) {
  ...
  const cellData = {
    cell_id: id,
    cell_name: "Cell Name", // Add the missing property
    cell_leader_id: "Cell Leader ID", // Add the missing property
    ...
    fellowships: [
      {
        cell_id: "ddf8cdca-38dd-45a1-91eb-4b11d4e03c17",
        member_id: id,
      ...
      {
        cell_id: "f5a8c7b9-e2d4-4f6a-b3c1-9d0e8m7k6j5h",
        member_id: id,
...
async function CellPage({ params }: { params: { id: string } }) {
  const cellData = await getZoneData(params.id)

  if (!cellData) {
    notFound()
  }

  return <CellDetails data={cellData} />
}
