...
const TransferModal = ({ 
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    const transferData = {
      member_id,
      new_cell_id: selectedCell.value, // Ensure this is the cell ID
      new_status: "TRANSFERRED" as "TRANSFERRED",
    ...
    try {
      await executeTransferMember(transferData);
    } catch (error:any) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button 
          className="bg-purple-500 text-white hover:bg-purple-900 rounded-lg px-4 py-2"
        >
          Transfer
        </Button>
      </DialogTrigger>
