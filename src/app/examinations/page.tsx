...
const Page = () => {
  ...
  useEffect(() => {
    if (cellResult?.data?.data && Array.isArray(cellResult.data.data)) {
      ...
      console.error("cellResult.data.data is not an array:", cellResult?.data?.data);
      setCellOptions([]); // Fallback to an empty array
    }
  }, [cellResult]);
    ...
    <div className="mx-auto w-[550px] mt-[100px] space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-2 mt-4">
          ...
          >
            {getCreateExamStatus === "executing" ? <Loader className="animate-spin text-sm mx-4" /> : "Create Examination"}
          </Button>
        </div>
      </form>
    </div>
